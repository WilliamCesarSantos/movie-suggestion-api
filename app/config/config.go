...
type RecommendationConfig struct {
	DefaultLimit               int
	MaxLimit                   int
	DefaultAlgorithm           entity.RecommendationAlgorithm
...
func Load() (*Config, error) {
	...
	defaultLimit, _ := strconv.Atoi(getEnv("RECOMMENDATION_DEFAULT_LIMIT", "10"))
	maxLimit, _ := strconv.Atoi(getEnv("RECOMMENDATION_MAX_LIMIT", "50"))
	hybridContent, _ := strconv.ParseFloat(getEnv("RECOMMENDATION_HYBRID_CONTENT_WEIGHT", "0.5"), 64)
	hybridCollab, _ := strconv.ParseFloat(getEnv("RECOMMENDATION_HYBRID_COLLABORATIVE_WEIGHT", "0.5"), 64)
	minRating, _ := strconv.ParseFloat(getEnv("RECOMMENDATION_MIN_IMDB_RATING", "6.0"), 64)
	serendipityRating, _ := strconv.ParseFloat(getEnv("RECOMMENDATION_SERENDIPITY_MIN_RATING", "5.0"), 64)
	contentBasedMin, _ := strconv.Atoi(getEnv("RECOMMENDATION_CONTENT_BASED_MIN_WATCHES", "5"))
	collaborativeMin, _ := strconv.Atoi(getEnv("RECOMMENDATION_COLLABORATIVE_MIN_WATCHES", "20"))
	contentPref, _ := strconv.ParseFloat(getEnv("RECOMMENDATION_CONTENT_PREFERENCE_THRESHOLD", "0.7"), 64)
