...
if err != nil {
log.Ctx(bgCtx).Error().Err(err).Str("term", term).Int("page", page).Msg("OMDB search error")
continue
}
if len(results) == 0 {
break
...
log.Ctx(bgCtx).Error().Err(err).Str("imdbId", r.ImdbID).Msg("failed to publish import message")
}
}
}
}
